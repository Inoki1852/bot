...
def editing(app, message):
    ...
    if len_text == 1:
        ...
        app.edit_message_text(chat.id, message.message_id, "<{0}><{1}>{2}</{3}></{4}>".format(text[0], text[1], message.text, text[1], text[0]))
    elif len_text == 3:
        app.edit_message_text(chat.id, message.message_id, "<{0}><{1}><{2}>{3}</{4}></{5}></{6}>".format(text[0], text[1], text[2], message.text, text[2], text[1], text[0]))
